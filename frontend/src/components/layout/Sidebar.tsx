...
export const Sidebar = ({ role, isOpen, onClose }: SidebarProps) => {
  ...
  let items = navItems[role] || navItems.owner;

  // Filter out admin-only sections if the user is an owner
  if (role === "owner" && currentUser?.role === "Owner") {
    items = items.filter(
      (item) => item.label !== "Fleet Validation" && item.label !== "Global Monitoring"
    );
  }
      ...
      >
        <div className="flex items-center justify-between mb-12">
          <div className="flex items-center gap-3">
            ...
            />
            <span className="font-black text-[13px] uppercase tracking-wider text-slate-400 border-l border-slate-200 pl-2 mt-0.5">
              Ops
            </span>
          </div>
